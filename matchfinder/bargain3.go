...
func (z *Bargain3) FindMatches(dst []Match, src []byte) []Match {
	...
	for i := historyLen; i < len(src); i++ {
		...
		if unmatched > 0 || i >= nextOverlapSearch || nextByteIsUnmatched {
			if int(candidate5.offset) < i && i-int(candidate5.offset) < z.MaxDistance &&
				uint32(cv) == candidate5.val &&
			...
			if int(candidate8.offset) < i && i-int(candidate8.offset) < z.MaxDistance &&
				uint32(cv) == candidate8.val &&
