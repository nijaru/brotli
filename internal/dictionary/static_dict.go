...
func isMatch(d *Dictionary, w DictWord, data []byte, max_length uint) bool {
	if uint(w.Len) > max_length {
		...
		if w.Transform == 0 {
			...
			return common.FindMatchLengthWithLimit(dict, data, uint(w.Len)) == uint(w.Len)
		} else if w.Transform == 10 {
			/* Match against uppercase first transform.
			   Note that there are only ASCII uppercase words in the lookup table. */
			return dict[0] >= 'a' && dict[0] <= 'z' && (dict[0]^32) == data[0] && common.FindMatchLengthWithLimit(dict[1:], data[1:], uint(w.Len)-1) == uint(w.Len-1)
		} else {
...
func FindAllStaticDictionaryMatches(dict *common.EncoderParams, data []byte, min_length uint, max_length uint, matches []uint32) bool {
	...
	// For now, let's assume we pass the Dictionary from common.
	d := dict.Dictionary
	var has_found_match bool = false
	var s []byte
	{
		var offset uint = uint(d.Buckets[hash(data)])
		var end bool = offset == 0
		for !end {
			w := StaticDictionaryWords[offset]
			offset++
			var l uint = uint(w.Len) & 0x1F
			var n uint = uint(1) << d.Words.(*Dictionary).Size_bits_by_length[l]
			var id uint = uint(w.Idx)
			end = !(w.Len&0x80 == 0)
			w.Len = byte(l)
			if w.Transform == 0 {
				var matchlen uint = dictMatchLength(d.Words.(*Dictionary), data, id, l, max_length)
				var minlen uint
				var maxlen uint
				var len uint
				...
				for len = minlen; len <= maxlen; len++ {
					var cut uint = l - len
					var transform_id uint = (cut << 2) + uint((d.CutoffTransforms>>(cut*6))&0x3F)
				...
				if !isMatch(d.Words.(*Dictionary), w, data, max_length) {
					continue
				}
	...
	if max_length >= 5 && (data[0] == ' ' || data[0] == '.') {
		...
		var offset uint = uint(d.Buckets[hash(data[1:])])
		var end bool = offset == 0
		for !end {
			w := StaticDictionaryWords[offset]
			offset++
			...
			if w.Transform == 0 {
				var s []byte
				if !isMatch(d.Words.(*Dictionary), w, data[1:], max_length-1) {
					continue
				}
	...
	if max_length >= 9 {
		...
		if (data[0] == ' ' && data[1] == 't' && data[2] == 'h' && data[3] == 'e' && data[4] == ' ') || (data[0] == '.' && data[1] == 'c' && data[2] == 'o' && data[3] == 'm' && data[4] == '/') {
			...
			for !end {
				...
				w.Len = byte(l)
				if w.Transform == 0 && isMatch(d.Words.(*Dictionary), w, data[5:], max_length-5) {
