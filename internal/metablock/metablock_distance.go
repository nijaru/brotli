...
type blockSplitterDistance struct {
	...
	num_blocks_        uint
	split_             *BlockSplit
	histograms_        []common.HistogramDistance
...
func initBlockSplitterDistance(self *blockSplitterDistance, alphabet_size uint, min_block_size uint, split_threshold float64, num_symbols uint, split *BlockSplit, histograms *[]common.HistogramDistance, histograms_size *uint) {
	...
	*histograms_size = max_num_types
	if histograms == nil || cap(*histograms) < int(*histograms_size) {
		*histograms = make([]common.HistogramDistance, *histograms_size)
	} else {
	...
	common.HistogramClearDistance(&self.histograms_[0])
...
func blockSplitterFinishBlockDistance(self *blockSplitterDistance, is_final bool) {
	...
	var histograms []common.HistogramDistance = self.histograms_
	...
	if self.num_blocks_ == 0 {
		...
		if self.curr_histogram_ix_ < *self.histograms_size_ {
			common.HistogramClearDistance(&histograms[self.curr_histogram_ix_])
		}
		self.block_size_ = 0
	} else if self.block_size_ > 0 {
		var entropy float64 = common.BitsEntropy(histograms[self.curr_histogram_ix_].Data_[:], self.alphabet_size_)
		var combined_histo [2]common.HistogramDistance
		...
		for j = 0; j < 2; j++ {
			...
			common.HistogramAddHistogramDistance(&combined_histo[j], &histograms[last_histogram_ix])
...
func blockSplitterAddSymbolDistance(self *blockSplitterDistance, symbol uint) {
	common.HistogramAddDistance(&self.histograms_[self.curr_histogram_ix_], symbol)
	self.block_size_++
