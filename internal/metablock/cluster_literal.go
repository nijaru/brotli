...
*/
func compareAndPushToQueueLiteral(out []common.HistogramLiteral, cluster_size []uint32, idx1 uint32, idx2 uint32, max_num_pairs uint, pairs []HistogramPair, num_pairs *uint) {
	...
	if out[idx1].Total_count_ == 0 {
		...
		var combo common.HistogramLiteral = out[idx1]
		var cost_combo float64
		common.HistogramAddHistogramLiteral(&combo, &out[idx2])
		cost_combo = common.PopulationCostLiteral(&combo)
...
func HistogramCombineLiteral(out []common.HistogramLiteral, cluster_size []uint32, symbols []uint32, clusters []uint32, pairs []HistogramPair, num_clusters uint, symbols_size uint, max_clusters uint, max_num_pairs uint) uint {
	...
	for num_clusters > min_cluster_size {
		...
		common.HistogramAddHistogramLiteral(&out[best_idx1], &out[best_idx2])
...
func HistogramBitCostDistanceLiteral(histogram *common.HistogramLiteral, candidate *common.HistogramLiteral) float64 {
	if histogram.Total_count_ == 0 {
		return 0.0
	} else {
		var tmp common.HistogramLiteral = *histogram
		common.HistogramAddHistogramLiteral(&tmp, candidate)
		return common.PopulationCostLiteral(&tmp) - candidate.Bit_cost_
	}
}
...
*/
func histogramRemapLiteral(in []common.HistogramLiteral, in_size uint, clusters []uint32, num_clusters uint, out []common.HistogramLiteral, symbols []uint32) {
	var i uint
	for i = 0; i < in_size; i++ {
		var best_out uint32
		if i == 0 {
	...
	for i = 0; i < num_clusters; i++ {
		common.HistogramClearLiteral(&out[clusters[i]])
	}

	for i = 0; i < in_size; i++ {
		common.HistogramAddHistogramLiteral(&out[symbols[i]], &in[i])
	}
}
...
func histogramReindexLiteral(out []common.HistogramLiteral, symbols []uint32, length uint) uint {
	var new_index []uint32 = make([]uint32, length)
	var next_index uint32
	var tmp []common.HistogramLiteral
	var i uint
	for i = 0; i < length; i++ {
	...
	tmp = make([]common.HistogramLiteral, next_index)
...
func ClusterHistogramsLiteral(in []common.HistogramLiteral, in_size uint, max_histograms uint, out []common.HistogramLiteral, out_size *uint, histogram_symbols []uint32) {
	...
	for i = 0; i < in_size; i++ {
		out[i] = in[i]
		out[i].Bit_cost_ = common.PopulationCostLiteral(&in[i])
