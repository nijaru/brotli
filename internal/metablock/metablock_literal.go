...
func initBlockSplitterLiteral(self *blockSplitterLiteral, alphabet_size uint, min_block_size uint, split_threshold float64, num_symbols uint, split *BlockSplit, histograms *[]common.HistogramLiteral, histograms_size *uint) {
	...
	*histograms_size = max_num_types
	...
	common.HistogramClearLiteral(&self.histograms_[0])
...
func blockSplitterFinishBlockLiteral(self *blockSplitterLiteral, is_final bool) {
	...
	if self.num_blocks_ == 0 {
		...
		if self.curr_histogram_ix_ < *self.histograms_size_ {
			common.HistogramClearLiteral(&histograms[self.curr_histogram_ix_])
		}
		self.block_size_ = 0
	} else if self.block_size_ > 0 {
		var entropy float64 = common.BitsEntropy(histograms[self.curr_histogram_ix_].Data_[:], self.alphabet_size_)
		var combined_histo [2]common.HistogramLiteral
		...
		for j = 0; j < 2; j++ {
			...
			common.HistogramAddHistogramLiteral(&combined_histo[j], &histograms[last_histogram_ix])
...
func blockSplitterAddSymbolLiteral(self *blockSplitterLiteral, symbol uint) {
	common.HistogramAddLiteral(&self.histograms_[self.curr_histogram_ix_], symbol)
	self.block_size_++
