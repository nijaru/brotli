...
func ensureInitialized(s *State) bool {
	...
	common.RingBufferSetup(&s.Params, &s.Ringbuffer_)

	/* Initialize last byte with stream header. */
	{
		lgwin := int(s.Params.Lgwin)
		if s.Plan.Tier == TierQ0 || s.Plan.Tier == TierQ1 {
			lgwin = max(lgwin, 18)
		}
...
func chooseContextMap(
	...
	if qLevel < MinQualityForHqContextModeling {
...
func shouldUseComplexStaticContextMap(
	...
	if Size_hint < 1<<20 {
		...
		var dummy uint
		var i uint
		utf8Lut := common.GetContextLUT(common.ModeUTF8)
		...
		for ; startPos+64 <= endPos; startPos += 4096 {
			...
			for pos = startPos + 2; pos < strideEndPos; pos++ {
				...
				ctx := byte(kStaticContextMapComplexUTF8[common.GetContext(prev1, prev2, utf8Lut)])
				total++
...
func decideOverLiteralContextModeling(
	...
	if qLevel < MinQualityForContextModeling || length < 64 {
		return
...
func writeMetaBlockInternal(
	...
	literalContextLut := common.GetContextLUT(literalContextMode)
	blockParams := *params
...
func chooseContextMode(
	...
	if params.Quality >= 10 &&
		!common.IsMostlyUTF8(data, pos, mask, length, common.KMinUTF8Ratio) {
		return common.ModeSigned
	}

	return common.ModeUTF8
}
...
func extendLastCommand(s *State, bytes *uint32, wrappedLastProcessedPos *uint32) {
	...
	distanceCode := CommandRestoreDistanceCode(lastCommand, &s.Params.Dist)
...
func copyInputToRingBuffer(s *State, inputSize uint, inputBuffer []byte) {
	common.RingBufferWrite(inputBuffer, inputSize, &s.Ringbuffer_)
