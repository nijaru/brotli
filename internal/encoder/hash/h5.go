...
func (h *h5) Initialize(params *common.EncoderParams) {
...
func (h *h5) StoreRange(data []byte, mask uint, ix_start uint, ix_end uint) {
	...
	for i+4 <= ix_end {
		k0 := uint32(binary.LittleEndian.Uint32(data[(i+0)&mask:])*KHashMul32) >> shift
		k1 := uint32(binary.LittleEndian.Uint32(data[(i+1)&mask:])*KHashMul32) >> shift
		k2 := uint32(binary.LittleEndian.Uint32(data[(i+2)&mask:])*KHashMul32) >> shift
		k3 := uint32(binary.LittleEndian.Uint32(data[(i+3)&mask:])*KHashMul32) >> shift
...
func (h *h5) StitchToPreviousBlock(
	num_bytes uint,
	position uint,
	ringbuffer []byte,
