...
func FindAllMatchesH10(
	...
	var dict_matches [common.MaxStaticDictionaryMatchLen + 1]uint32
	var i uint
	...
	for i = 0; i <= common.MaxStaticDictionaryMatchLen; i++ {
		dict_matches[i] = common.KInvalidMatch
	}
	{
		...
		if common.FindAllStaticDictionaryMatches(
			params,
			data[cur_ix_masked:],
			minlen,
			max_length,
			...
			var maxlen uint = min(common.MaxStaticDictionaryMatchLen, max_length)
			var l uint
			for l = minlen; l <= maxlen; l++ {
				var dict_id uint32 = dict_matches[l]
				if dict_id < common.KInvalidMatch {
