...
func initialEntropyCodesCommand(
	data []uint16,
	length uint,
	stride uint,
	num_histograms uint,
...
func randomSampleCommand(
	seed *uint32,
	data []uint16,
	length uint,
	stride uint,
	sample *common.HistogramCommand,
) {
	var pos uint = 0
	if stride >= length {
		stride = length
...
func refineEntropyCodesCommand(
	data []uint16,
	length uint,
	stride uint,
	num_histograms uint,
